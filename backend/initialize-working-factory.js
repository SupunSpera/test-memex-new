...
async function initializeWorkingFactory() {
  try {
    ...
    console.log('🔧 Initializing the working Factory contract...');
    ...
    console.log(`Foundry: ${addresses.Foundry}`);

    const factoryABI = [
      "function initialize(uint256 factoryFees, address owner, address tokenImpl, address bondingCurveImpl, address lockContractAddr, tuple(uint256 virtualEth, uint256 bondingTarget, uint256 minContribution, uint24 poolFee, uint24 sellFee, address uniswapV3Factory, address positionManager, address weth, address feeTo) initialSettings) external",
    ...
    // Initialize with proper settings (simplified - no prebonding)
    ...
    console.log(`- Position Manager: ${initialSettings.positionManager}`);
    
    // Call initialize function
    const tx = await factory.initialize(
      ethers.parseEther("0.0001"), // 0.0001 ETH deployment fee (matching your original)
